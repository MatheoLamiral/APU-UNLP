...
type
  ...
  Procedure baja(var arch:archivoEmpleados);
  var 
   e:empleado;
   num,pos:integer;
  begin
    writeln('ingrese un numero de empleado para darlo de baja del archivo: ');
    readln(num);
    if(not eof(arch))then Read(arch,e);
    while(not eof(arch))and(e.num<>num)do
    begin
    ...
    begin
        ...
        seek(arch,filesize(arch)-1);
        truncate(arch);
        writeln('Empleado dado de baja con exito!');
    end
    else writeln('El empleado con el numero ingresado no existe!');
  end;
