...
>[!important]
> - Para calcular la cantidad de host máximos en una subred, se utiliza la fórmula 2^(32 - n) - 2, donde n es la cantidad de bits asignados a la parte de red (máscara de subred). Se resta 2 para excluir la dirección de red y la dirección de broadcast, que no pueden ser asignadas a hosts.
> - La dirección de red es la primera dirección en una subred, donde todos los bits de la parte de host son 0, y la de broadcast es la última dirección en una subred, donde todos los bits de la parte de host son 1.
...
- `172.16.58.223/26`
  ...
  - **Cantidad máxima de hosts**: La cantidad máxima de hosts que se pueden representar son 2^(32-26) - 2 = 62 hosts
  - **Dirección de broadcast**: La dirección de broadcast es `172.16.58.255`
...
- `163.10.5.49/27`  
  ...
  - **Cantidad máxima de hosts**: La cantidad máxima de hosts que se pueden representar son 2^(32-27) - 2 = 30 hosts
  - **Dirección de broadcast**: La dirección de broadcast es `163.10.5.255`
...
- `128.10.1.0/23`
  ...
  - **Cantidad máxima de hosts**: La cantidad máxima de hosts que se pueden representar son 2^(32-23) - 2 = 510 hosts
  - **Dirección de broadcast**: La dirección de broadcast es `128.10.0.255`
...
- `10.1.0.0/24`
  ...
  - **Cantidad máxima de hosts**: La cantidad máxima de hosts que se pueden representar son 2^(32-24) - 2 = 254 hosts
  - **Dirección de broadcast**: La dirección de broadcast es `10.1.0.255`
...
- `8.40.11.179/12`
  ...
  - **Cantidad máxima de hosts**: La cantidad máxima de hosts que se pueden representar son 2^(32-12) - 2 = 1048574 hosts
  - **Dirección de broadcast**: La dirección de broadcast es `8.32.0.255`
...
- Dada la dirección IP `128.50.10.0`
  ...
  - **Cantidad de hosts posibles**
    - La cantidad máxima de hosts que se pueden representar son 2^(32-16) - 2 = 65534 hosts
- Si se necesitan crear, al menos, 513 subredes:
  ...
  - **Cantidad de redes asignables**:
    - Después de tomar 10 bits de host, podemos representar 2^10 = 1024 subredes.
  ...
  - **Dirección de la subred 710**:
    - Sabemos que tenemos 10 bits para representar las subredes, porque pasamos de `/16` a `/26`. Es decir, los primeros 16 bits son de red y los siguientes 10 bits son de subred.
    - Como nos piden la red 710 y se empieza a contar desde 0, calcularmos la representación binaria de 709:
      - 709 en binario es `1011000101`
    - Ahora aplicamos un OR entre la dirección IP inicial y la representación binaria de la subred 710, teniendo en cuenta que los primeros 16 bits son de red y los siguientes 10 bits son de subred:
      - `128.50.[00001010.00000000] OR 128.50.[10110001.01000000] = 128.50.10110001.01000000`
      - En decimal sería `128.50.177.64/26`
  - **Dirección de broadcast de la subred 710**:
    ...
    - Como la máscara es `/26`, los primeros 26 bits son de red y los últimos 6 bits son de host.
      - `128.50.10110001.01111111` sería la dirección de broadcast en binario.
      - En decimal sería `128.50.177.127/26`
...
- **Máscara para definir al menos 9 subredes**
  ...
  - Por lo tanto, la máscara necesaria será `24 + 4 = /28`.
...
CIDR significa **Classless Interdomain Routing** (Enrutamiento entre Dominios sin Clase), es una **estrategia de asignación de direcciones que elimina las clases (Classless), y determina que la parte de red de una dirección IP puede tener longitudes arbitrarias**. En el esquema CIDR, una dirección IP se expresa en notación decimal con punto como `a.c.c.d/x`, donde el valor x (la longitud del prefijo) indica el número de bits más significativos (de izquierda a derecha) que constituyen la parte de red o prefijo de la dirección IP.  
...
- Básicamente, permite máscaras de subred de longitud variable (VLSM), lo que optimiza la asignación de direcciones IP y utiliza resumen de rutas para reducir el tamaño de las tablas de ruteo.
...
- Dadas los siguientes bloques CIDR, las redes involucradas son
  - `200.56.168.0/21`
    - en binario: [11001000.00111000.10101]000].00000000
    - por ende, el rango de redes involucradas va desde `200.56.168.0/24` hasta `200.56.175.0/24`
  - `195.24.0.0/13`
    - en binario: [11000011.00011]000.00000000].00000000 red actualmente con los últimos 19 bits en 0 y el máximo con los últimos 19 bits en 1
    - por ende, el rango de redes involucradas va desde `195.24.0.0/24` hasta `195.31.255.0/24`
  - `195.24/13`
    - es lo mismo que el punto anterior, por lo que el rango de redes involucradas va desde `195.24.0.0/24` hasta `195.31.255.0/24`

>[!note]
> Hay una explicación de como calcular las redes involucradas usando como ejemplo el primer punto en el archivo [CIDR.pdf](CIDR.pdf)
...
Está hecho en el punto anterior.
...
- manipulación del campo TTL:
  - El campo TTL (Time To Live) es un campo de 8 bits en el encabezado IP, cuyo propósito es evitar bucles infinitos en la red. **Cada vez que un paquete pasa por un router, el TTL se reduce en 1**, y **si llega a 0**, el **router descarta el paquete** y envía un mensaje **ICMP de tipo 11 y código 0 (tiempo excedido) al remitente**. De esta manera, el programa recibe una respuesta de cada router intermedio y puede identificar la ruta completa hasta el destino, que responde finalmente con un mensaje ICMP “Echo Reply” (tipo 0, código 0) o “Port Unreachable” (tipo 3, código 0-15) cuando el paquete logra llegar
