...
- ¿Dónde se unen nuevamente los fragmentos? ¿Qué sucede si un fragmento no llega?
  - Los paquetes se unen nuevamente en el host de destino, en este caso PC2. Si un fragmento no llega, el accionar dependerá del protocolo de capa de transporte que se utilice.
...
El ruteo se refiere al proceso que realiza la red en su conjunto para **determinar la ruta o camino que deben seguir los paquetes a medida que fluyen de un emisor a un receptor**. Se considera una **función de control**, es decir, que no maneja directamente el tráfico de datos, sino que toma decisiones sobre como se debería enrutar el tráfico. Los algoritmos de enrutamiento calculan estas rutas. El ruteo se encarga de determinar los valores que se introducen en las tablas de reenvío de los routers. Un router despacha mensajes consultando su tabla de ruteo, desde cualquier interfaz. La tabla de ruteo tiene una estructura que incluye la **Red Destino** (Net/Mask), el **Próximo Salto** (Next Hop) y la **Interfaz de salida**. El ruteo es necesario porque es fundamental para cumplir la función principal de la Capa de Red, **transportar paquetes desde un host emisor a un host receptor**

### Ejercicio 3

- Ruteo estático:
  - Los algoritmos de enrutamiento estático son aquellos en los que las rutas cambian muy lentamente a lo largo del tiempo. Las rutas se configuran manualmente, a menudo como resultado de una intervención humana.
  - Ventajas
...
- La tabla anterior no tiene salida a internet ya que no tiene ninguna entrada que lleve a un ISP.
- Lo solucionaría confiugando un default getaway(0.0.0.0/0) por Rtr-C en la tabla de ruteo.
    |Red Destino|Máscara|Next-Hop|Iface|
    |-----------|-------|--------|-----|
    |...|...|...|...|
...
Si Rtr-C tuviese esa entrada y un equipo en Red D intenta acceder a `163.10.5.15`, se producirá un bucle de ruteo. El paquete saldrá de Red D hacia Rtr-D, Rtr-D lo enviará a Rtr-C, y Rtr-C, por configurado como Next-Hop para `163.10.5.0` a Rtr-D, reenviará el paquete a Rtr-D. Esto se repetirá sucesivamente hasta que el TTL expire, por lo que no habrá comunicación con `163.10.5.15`
...
- Sumarización
  ...
  - Consideraciones para sumarizar
    ...
    - La primera dirección IP debe ser divisible por la suma de los host de los bloques a sumarizar
      - Si la dirección IP termina en 0, siempre va a ser divisible
...
- Podemos aplicar sumarización para las siguientes entradas en la tabla de ruteo de Rtr-D:
  ...
  - La primera dirección IP es divisible por la suma de los host ya que cada /26 tiene 64 host, y al sumarizar tenemos un rango de 128 hosts (64+64). La primera dirección `205.20.0.128` es divisible por 128.
...
La sumarización aplicada en el punto anterior no se podría aplicar en el Rtr-B ya que las redes `205.20.0.128` y `205.20.192` usan interfaces diferentes, eth0 y eth1 respectivamente, y por lo tanto no cumplen con el último requisito para poder sumarizar.
...
- **Mensaje ICMP enviado por PC-B a PC-C**
  ...
  - tipo de respuesta recibida en el emisor
    - La respuesta que recibira esta de tipo 0 con código 0 (Echo Reply)
- **Mensaje ICMP enviado por PC-C a PC-B**
  - llegada a destino?
    ...
    - saltos que tomará
      ...
      - router 4 recibe el mensaje y como tiene una entrada para `10.0.0.0/16`, que tiene como next hop a router2, lo reenvía a router1 a través de la Iface eth0 usando el Next-Hop `10.0.1.1`
...
Al eliminar el archivo `/var/lib/dhcp/dhclient.leases`, el cliente DHCP pierde el registro de su asignación anterior. Por ese motivo, cuando se ejecuta nuevamente dhclient, el cliente no puede solicitar la misma IP anterior y comienza el proceso completo de DHCP como si fuera la primera vez, es decir realiza el proceso completo (Discover → Offer → Request → ACK). En la captura de Wireshark se observa un intercambio diferente al del punto b:
...
El **NAT (Network Address Translation, o Traducción de Direcciones de Red)** es un proceso que realiza la traslación de direcciones IP de un espacio privado (no enrutable en Internet) a un espacio público. Los procesos de traslación see realizan sobre redes stubs (solo una salida) y se deben mantener tablas de traslaciones.
